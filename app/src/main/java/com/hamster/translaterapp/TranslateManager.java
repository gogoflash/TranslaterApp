...
public class TranslateManager implements Callback<YandexTranslaterResponseJSON> {
    ...
    private static final String API_KEY = "trnsl.1.1.20170405T164546Z.927ade72a74e3dde.fa3a85a19ee0ef151eb43fca91feb6a6cf9600f6";
    ...
    {
        // Эти строчки кода повторяется в TranslaterModel.translate(), но тут нужна лишняя проверка, чтобы не расставлять потенциальным коллегам из команды кодовые мины.
        if(text == null || text.equals(""))
            return;
        text = text.trim();

        Log.d("translaterApp", "requestTranslate: " + text + " " + languageCode);
        // вырубаем предыдущий запрос, если тот еще не выполнился:
        if(currentCall != null)
        {
        ...
        // новый запрос:
        currentCall = yandexService.search(API_KEY, text, languageCode, null, null);
    ...
    public void requestGetLanguagesList()
    {
        ...
        Call<YandexTranslaterLanguagesResponseJSON> call = yandexLanguagesListService.search(API_KEY, languageCode);
